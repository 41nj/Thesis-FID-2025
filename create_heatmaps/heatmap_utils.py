...
from create_heatmaps.resample_utils import wasserstein2_loss


class ActivationsAndGradients:
    """Class to obtain intermediate 
    vations and gradients.
       Adapted from: https://github.com/jacobgil/pytorch-grad-cam"""

    def __init__(self, network, network_kwargs, target_layer_name='Mixed_7c'):
        self.network = network
        self.network_kwargs = network_kwargs
        self.gradients = []
        self.activations = []

        # Direkt auf das Modul zugreifen
        target_layer = dict(self.network.named_modules())[target_layer_name]
...
def compute_sensitivity_heatmap(gen_image: torch.Tensor,
    ...
    #print(features.device)  # Zeigt das Gerät von `features`
    #print(mean_gen.device)
    #----------------------------------------------------------------------------
    ...
    print(f"mean_reals min: {mean_reals.min()}, max: {mean_reals.max()}")
    print(f"cov_reals min: {cov_reals.min()}, max: {cov_reals.max()}")
    print(f"mean_gen min: {mean.min()}, max: {mean.max()}")
    print(f"cov_gen min: {cov.min()}, max: {cov.max()}")
    ...
    print(f"activations shape: {activations.shape}, gradients shape: {gradients.shape}")
    print(f"activations min: {activations.min()}, max: {activations.max()}")
    print(f"gradients min: {gradients.min()}, max: {gradients.max()}")


    #print(f"gen_image shape: {gen_image.shape}, device: {gen_image.device}")
    ...
    print(f"sensitivity_map type: {type(sensitivity_map)}, shape: {sensitivity_map.shape}")
    print(f"sensitivity_map min: {sensitivity_map.min()}, max: {sensitivity_map.max()}")
    ...
    sensitivity_map = zero_one_scaling(image=sensitivity_map)
    print(f"After zero_one_scaling - min: {sensitivity_map.min()}, max: {sensitivity_map.max()}")
    ...
    print(f"sensitivity_map resized shape: {sensitivity_map.shape}")
