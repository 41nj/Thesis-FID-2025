...
from create_heatmaps.metrics.frechet_inception_distance import compute_fid
from create_heatmaps.metrics.kernel_inception_distance import compute_kid
...
def wasserstein2_loss(mean_reals: torch.Tensor,
    ...
    eigenvalues, _ = torch.linalg.eig(torch.matmul(cov_gen, cov_reals))
    eigenvalues = eigenvalues.real + 1e-6  

    cov_term = torch.trace(cov_reals) + torch.trace(cov_gen) - 2 * torch.sum(torch.sqrt(eigenvalues + eps))
    return mean_term + cov_term"""
    mean_term = torch.sum(torch.square(mean_reals - mean_gen.squeeze(0)))

    # Eigenvalue calculation with numerical stabilization
    # Eigenvalues and eigenvectors of the matrix product of cov_gen and cov_reals
    eigenvalues, _ = torch.linalg.eig(torch.matmul(cov_gen, cov_reals))
    eigenvalues = eigenvalues.real  # Only the real part of eigenvalues is taken
    eigenvalues = torch.clamp(eigenvalues, min=eps)  # Ensure no negative or too small eigenvalues

    # Kovariance term: Adding eps to the diagonal of the covariance matrices to prevent instability
    cov_term = torch.trace(cov_reals) + torch.trace(cov_gen) - 2 * torch.sum(torch.sqrt(eigenvalues + eps))
    
    # Return the total Wasserstein loss
    return mean_term + cov_term
